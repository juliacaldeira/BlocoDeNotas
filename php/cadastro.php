...
if( (!empty($_POST['nome'])) && (!empty( $_POST['username'])) && (!empty( $_POST['email'])) && (!empty( $_POST['senha'])) ) {
    ...
    $verifUsername = mysqli_query($conexao, "SELECT * FROM user WHERE username = '$username'");
    $verifEmail = mysqli_query($conexao, "SELECT * FROM user WHERE email = '$email'");

    $linhasUsername = mysqli_num_rows($verifUsername);
    $linhasEmail = mysqli_num_rows($verifEmail);
    
    if((!$linhasUsername > 0) && (!$linhasEmail > 0)) {
        $string_sql = "INSERT INTO user (user_id, name, username, email, senha) VALUES (null, '$nome', '$username', '$email', '$senha')";
